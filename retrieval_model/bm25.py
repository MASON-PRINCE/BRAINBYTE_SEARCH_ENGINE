...
def retrieve_function(query, precision=0.05):
    ...
    top_doc_indices = np.argsort(-total_score)[:500]
    top_doc_ids = [total_ids[i] for i in top_doc_indices]
    top_doc_scores = [total_score[i] for i in top_doc_indices]
    top_doc = []
    for i, (doc_id, score) in enumerate(zip(top_doc_ids, top_doc_scores)):
...
def sample_retrieve_function(query):
    ...
    filtered_scores = bm25_scores[:, q_index]
    doc_scores_sum = np.array(filtered_scores.sum(axis=1)).flatten()
    top_doc_indices = np.argsort(-doc_scores_sum)[:200]
    top_doc_ids = [ids[i] for i in top_doc_indices]
    top_doc_scores = [doc_scores_sum[i] for i in top_doc_indices]
    top_doc = []
