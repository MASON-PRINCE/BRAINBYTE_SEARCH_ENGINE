...
from retrieval_model.predict import predict_function
...
def search_api(query, test):
    results = predict_function(query, test=test)
    r, res_dic = {}, {}
...
class SearchThread(QThread):
    ...
    def __init__(self, query, test, parent=None):
        super(SearchThread, self).__init__(parent)
        self.query = query
        self.test = test

    def run(self):
        # Perform the search operation with the selected 'test' value
        results = search_api(self.query, self.test)
...
class WatermarkLabel(QLabel):
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(textRect, Qt.AlignmentFlag.AlignCenter, '')
        painter.end()
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.logoPixmap = QPixmap('qmul_logo.png')
        ...
        # Create the dropdown to select 'test' parameter value
        self.testDropdown = QComboBox()
        self.testDropdown.addItem('Test Set')  # Corresponds to test=True
        self.testDropdown.addItem('Full Data')  # Corresponds to test=False
        self.searchLayout.addWidget(self.testDropdown)
    ...
    def onSearchClicked(self):
        ...
        selected_option = self.testDropdown.currentText()
        test_value = selected_option == 'Test Set'  # 'fast' corresponds to True, 'accurate' to False

        if test_value and query not in queries_set:
        ...
        # Hide search window while searching
